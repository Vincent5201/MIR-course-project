...
def evaluate_clap_pipeline(csv_path, audio_dir, sample_ratio=0.1, threshold=0.5):
    ...
    print("-" * 60)
    print(f" 🎯 歌曲標籤組合精準度指標 (Sample-level, th={threshold:.1f}):")
    print(f"   - 完全命中率 (Subset Accuracy)       : {subset_accuracy * 100:.2f}%  (所有標籤全對才算對)")
    print(f"   - 平均歌曲得分 (Sample-level F1)     : {mean_sample_f1 * 100:.2f}%  (綜合懲罰多預測與少預測)")
    print("=====================================================================")
...
if __name__ == "__main__":
    ...
    evaluate_clap_pipeline(CSV_PATH, AUDIO_DIR, sample_ratio=1, threshold=0.57)